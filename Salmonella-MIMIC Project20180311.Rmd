...
glmProjectCnt <- glmProjectDF %>%
  ...
  filter(n>100) #for n>10, 16864 instead of ~60k --> only cuts down total by 100,000
...
glmProjectDF <- na.omit(glmProjectDF, cols="word") #nothing removed
...
#projectMatrix <- na.omit(projectMatrix) #doesn't change anything
printSpMatrix2(projectMatrix)
...
```{r spelling}
spellcheckedDF <- unique(subsetTokens[ ,6]) #All unique words
spellcheckedDF$wordFixed <- spellcheckedDF$word
#write.csv(spellcheckedDF, file="C:/Users/Maggie/OneDrive/UW - BHI/2018 Winter/BIOS 544/data/projectSubsetTerms.csv", sep=",", row.names=F, col.names = T)

#http://housesofstones.github.io/2012/09/13/automatic-cleaning-of-messy-text-data/
library(devtools)
install_github("tmt", "schaunwheeler") #didn't work
aspellCheck(spellcheckedDF$wordFixed, "fix")


library(hunspell)
correctWords <- hunspell(spellcheckedDF$wordFixed)
...
spellCheck <- check_spelling_interactive(spellcheckedDF$wordFixed)
preprocessed(spellCheck)
fixit <- attributes(spellCheck)$correct
fixit(spellcheckedDF$wordFixed)
